...
const orderSchema = z.object({
  ...
  wa: z.string().regex(/^08\d{8,11}$/, { message: "Format WhatsApp tidak valid. Gunakan format: 08xxx" }),
  jenis: z.enum(["Makalah", "PPT", "Artikel"], { required_error: "Pilih jenis tugas." }),
  halaman: z.coerce.number().min(1, { message: "Minimal 1 halaman." }).max(500, { message: "Maksimal 500 halaman." }),
        ...
        <div className="max-w-2xl mx-auto py-12">
          <Card className="border-green-200 bg-green-50 shadow-sm">
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="text-2xl text-green-800">Order Berhasil Dibuat!</CardTitle>
            ...
            <CardContent className="flex flex-col items-center space-y-6 pt-6">
              <div className="bg-white px-8 py-6 rounded-xl border border-green-100 text-center w-full max-w-sm">
                <p className="text-sm text-slate-500 mb-2 font-medium">ORDER ID ANDA</p>
                <div className="flex items-center justify-center gap-3">
                  <span className="text-2xl font-bold tracking-wider text-slate-900">{successOrderId}</span>
                  <Button variant="outline" size="icon" onClick={copyToClipboard} className="h-8 w-8 text-slate-500 hover:text-slate-900">
              ...
              <Alert className="bg-yellow-50 border-yellow-200 text-yellow-800">
                ...
                <AlertDescription>
                  Harap catat dan simpan Order ID ini. Anda akan membutuhkannya untuk mengecek status pesanan Anda.
                </AlertDescription>
              </Alert>
              
              <div className="flex gap-4 w-full justify-center pt-4">
                <Button variant="outline" onClick={() => { setSuccessOrderId(null); form.reset(); }}>
                  Order Baru Lagi
                </Button>
                <Button onClick={() => window.location.href = `/track?id=${successOrderId}`}>
                  Lacak Order Ini
                </Button>
              </div>
            </CardContent>
      ...
      <div className="max-w-2xl mx-auto py-8">
        <Card className="border-slate-200 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Isi detail tugas Anda dengan lengkap agar kami dapat memberikan estimasi dan hasil terbaik.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>Untuk komunikasi & pengiriman file.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                ...
                <Button type="submit" className="w-full h-12 text-lg" disabled={createOrder.isPending}>
